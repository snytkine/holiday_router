...
describe('#pathparamnode.ts', () => {
  describe('#PathParamNode object test', () => {
    ...
    it('Created instance should be an instance of PathParamNode', () => {
      ...
      expect(nodeWithPrefixAndPostfix)
      .to
      .be
      .instanceOf(PathParamNode)
    })
    ...
    it('PathParamNode should have priority of PRIORITY.PATHPARAM', () => {
      ...
      .to
      .equal(getNodePriority(PRIORITY.PATHPARAM))

      expect(nodeWithPrefixAndPostfix.priority)
      .to
      .be
      .greaterThan(getNodePriority(PRIORITY.PATHPARAM))
    })
    ...
    it('PathParamNode should have name type TAG.PATHPARAM_NODE', () => {
      ...
      expect(nodeWithPrefixAndPostfix.type)
      .to
      .equal(TAG.PATHPARAM_NODE)
    })
    ...
    it('equals for node with no prefix and no postfix should be true if other node is PathParamNode', () => {
      expect(node1.equals(new PathParamNode('var1')))
      .to
      .equal(true)
    })
    ...
    it('equals for node with prefix and postfix should be true if other node is PathParamNode with same prefix and postfix', () => {
      expect(nodeWithPrefixAndPostfix.equals(new PathParamNode('var1', '.html', 'order-')))
      .to
      .equal(true)
    })
    ...
    it('equals for node with prefix and postfix should be false if other node is PathParamNode with same postfix and different prefix', () => {
      expect(node1.equals(new PathParamNode('var1', '.html')))
      .to
      .equal(false)
    })
    ...
    it('equals for node with no prefix and no postfix should be false if other node is NOT PathParamNode', () => {
      expect(node1.equals(new ExactMatchNode('var1')))
      .to
      .equal(false)
    })
    ...
    it('.findRoutes should return iterator with all matches', () => {
      ...
      expect(route1.value.node)
      .to
      .equal(nodeWithPrefixAndPostfix)

      expect(route1.value.controller)
      .to
      .equal(ctrl)

      expect(route1.value.params.pathParams[0].paramName)
      .to
      .equal('id')

      expect(route1.value.params.pathParams[0].paramValue)
      .to
      .equal('1234')

      expect(route2.value.node)
      .to
      .equal(nodeWithPrefixAndPostfix)

      expect(route2.value.controller)
      .to
      .equal(ctrl2)
    })
    ...
    it('.findRoute uri should return single RouteMatch with highest priority', () => {
      ...
      const route = <IRouteMatch<BasicController<string>>>nodeWithPrefixAndPostfix.findRoute('order-1234.html');
  ...
  describe('#PathParamNode with child nodes test', () => {
    ...
    it('findRoute should extract pathParams from url and find from child node', () => {
      ...
      expect(route.controller.id)
      .to
      .equal('ctrlX')

      expect(route.params.pathParams)
      .to
      .deep
      .include({
        paramName:  'id',
