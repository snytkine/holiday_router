...
describe('#pathparamnoderegex node', () => {
  ...
  describe('#PathParamNodeRegex makeUri test', () => {
    ...
    it('.findRoutes should return iterator with all matches', () => {
      const nodeWithPrefixAndPostfix = new PathParamNodeRegex('id', new RegExp('([0-9]+)'), '.html', 'order-');
    ...
    it('PathParamNodeRegex with child nodes .findRoutes should return iterator from child nodes with all matches', () => {
      const nodeWithPrefixAndPostfix = new PathParamNodeRegex('id', new RegExp('([0-9]+)'), '/', 'order-');
      ...
      expect(route1.value.params.pathParams[1].paramName)
      .to
      .equal('name')

      expect(route1.value.params.pathParams[1].paramValue)
      .to
      .equal('NICK')
