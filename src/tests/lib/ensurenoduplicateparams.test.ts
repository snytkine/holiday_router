  ...
  describe('#ensureNoDuplicateParams', () => {
    ...
    it('#Must throw error when any of the parent nodes has same paramName', () => {
      try {
        ...
        throw new Error(
          'ensureNoDuplicatePathParams must throw error for non-unique paramName "id"',
        );
      } catch (e) {
        expect(e.code).to.equal(RouterErrorCode.NON_UNIQUE_PARAM);
        expect(e.message).to.equal(
          `URI params must be unique. Non-unique param "id" found in node=PathParamNode::id::'order-'::'.html'`,
        );
      }
    });
    ...
    it('#Must throw error when any of the parent nodes has same paramName', () => {
      try {
        ...
        expect(e.code).to.equal(RouterErrorCode.NON_UNIQUE_PARAM);
        expect(e.message).to.equal(
          `URI params must be unique. Non-unique param "person" found in node=PathParamNode::person::''::'/'`,
        );
      }
    });
